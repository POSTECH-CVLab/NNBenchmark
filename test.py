...
def match(method, n, d, k, num_iter):
  ...
  for x, y in dataset:
    total_timer.tic()
    prepare_timer.tic()
    tree, query = method.prepare_input(x, y, k)
    prepare_timer.toc()

    match_timer.tic()
    result = method.match(tree, query, k)
    match_timer.toc()
...
def sanity_check(method):
  ...
  for x, _ in dataset:
    ...
    if np.sum(dists[:, 0]) > 0 or not np.array_equal(inds[:, 0], np.arange(0, n)):
      is_pass = False
...
if __name__ == "__main__":
  ...
  parser.add_argument('--max_scale', type=int, default=9, help='log_10(max scale)')
  parser.add_argument('--max_dim', type=int, default=1, help='log_2(max dim')
  parser.add_argument('--max_knn', type=int, default=8, help='log_2(max knn)')
  ...
  if opt.sanity:
    ...
    k_list = [pow(2, k) for k in range(0, opt.max_knn)]

    sweep = list(itertools.product(n_list, d_list, k_list))

    results = []
    for n, d, k in sweep:
      result = match(method, n, d, k, opt.num_iter)
      print("N %d, D %d, K %d, prepare time %.4f, matching time %.4f, total time %.4f" %
            result)
    ...
    df = pd.DataFrame(results, columns=['n', 'd', 'k', 'prepare', 'match', 'total'])
    df.to_csv(filename)
